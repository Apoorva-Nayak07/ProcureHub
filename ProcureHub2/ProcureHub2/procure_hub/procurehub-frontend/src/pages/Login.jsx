...
import { faEnvelope, faLock, faEye, faEyeSlash, faIndustry, faClipboardList } from "@fortawesome/free-solid-svg-icons";
...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { token, role, company_name } = res.data;
      localStorage.setItem("token", token);
      localStorage.setItem("role", role);
      localStorage.setItem("company_name", company_name || "");
      navigate(role === "vendor" ? "/marketplace" : "/dashboard");
    } catch (err) {
      const msg = err?.response?.data?.message || "Login failed. Is the backend running?";
      setError(msg);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.card}>
        ...
        <p style={s.sub}>Sign in to your procurement account</p>

        <div style={s.roleToggle}>
          {[
            { val: "vendor",              label: "Vendor",  icon: faIndustry },
            { val: "procurement_manager", label: "Manager", icon: faClipboardList },
          ].map((r) => (
            <button key={r.val} type="button"
              style={{ ...s.roleBtn, ...(form.role === r.val ? s.roleBtnActive : {}) }}
              onClick={() => setForm({ ...form, role: r.val })}>
              <FontAwesomeIcon icon={r.icon} style={{ marginRight: 6 }} />
              {r.label}
            </button>
          ))}
        </div>
          ...
          <div style={s.field}>
            ...
            <div style={s.inputWrap}>
              ...
              <input className="input-base" style={s.input} type={showPass ? "text" : "password"}
                name="password" value={form.password} onChange={handle} placeholder="••••••••" required />
        ...
        <p style={s.footer}>
          New to ProcureHub? <Link to="/register" style={s.footerLink}>Create account</Link>
        </p>
      </div>
    </div>
  );
...
const s = {
  page:          { minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(145deg,#2E3528 0%,#1C2118 50%,#252C1E 100%)", position: "relative", overflow: "hidden" },
  ...
  card:          { position: "relative", background: "#fff", borderRadius: 20, padding: "40px 36px", width: 420, boxShadow: "0 24px 80px rgba(0,0,0,0.35)", animation: "scaleIn 0.35s cubic-bezier(0.4,0,0.2,1)" },
  ...
  roleToggle:    { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, background: "var(--bg-alt)", borderRadius: 10, padding: 4, marginBottom: 24 },
  roleBtn:       { padding: "9px", borderRadius: 8, border: "none", background: "transparent", fontSize: 13, fontWeight: 500, color: "var(--text-muted)", cursor: "pointer", transition: "all 0.2s" },
  roleBtnActive: { background: "#fff", color: "var(--moss)", fontWeight: 700, boxShadow: "var(--shadow-sm)" },
  ...
  submitBtn:     { marginTop: 4, padding: "13px", background: "linear-gradient(135deg,#6B7F5E,#516349)", color: "#fff", border: "none", borderRadius: 10, fontSize: 15, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, boxShadow: "0 4px 16px rgba(107,127,94,0.40)", cursor: "pointer" },
